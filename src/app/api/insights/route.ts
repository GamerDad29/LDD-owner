...
const PROMPTS: Record<string, (data: any) => string> = {
  overview: (d) =>
    `Sales period: ${d.days} days, Jan 1–Feb 23 2026. Revenue: $${d.totalSales.toFixed(0)}, Orders: ${d.totalOrders}, AOV: $${d.aov.toFixed(2)}, Best day: $${d.bestDay.toFixed(0)}. YoY vs 2025 same period: revenue +${d.yoySales.toFixed(1)}%, orders +${d.yoyOrders.toFixed(1)}%. Give me a 2-sentence executive summary and the single most important action item.`,
  ...
  aov: (d) =>
    `Daily AOV trend over ${d.length} days (2026 YTD). Range: $${Math.min(...d.map((x: any) => x.aov)).toFixed(0)}–$${Math.max(...d.map((x: any) => x.aov)).toFixed(0)}, avg: $${(d.reduce((s: number, x: any) => s + x.aov, 0) / d.length).toFixed(2)}. What's causing fluctuation and how can Rebecca raise the floor on low-AOV days?`,
}
